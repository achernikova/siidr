...
for variant in variants:
    ...
    sim.simulation_over_param_grid(nsim, 'siidr')
    sim.get_best_models()
...
for variant in variants:
    ...
    # simulate
    sim = Simulation(param_grid, actual_I, k)
    sim.simulation_over_param_grid(nsim, 'sir')
    sim.get_best_models()
    model2[variant] = sim
...
beta   = np.linspace(start = 0.01, stop = 0.99, num = 20)
mu     = np.linspace(start = 0.01, stop = 0.99, num = 20)
gamma = np.linspace(start = 0.01, stop = 0.99, num = 20)
gamma1 = [0]
gamma2 = [0]
R0     = [0]
I0     = [1]
...
for variant in variants:
    ...
    sim.simulation_over_param_grid(nsim, 'seir')

    sim.get_best_models()

    model5[variant] = sim

    print('best aic', min(sim.aic))
    print('best params beta', sim.best_params['beta'])
    print('best params mu', sim.best_params['mu'])
    print('best params gamma', sim.best_params['gamma'])
...
for variant in variants:
    ...
    sim.simulation_over_param_grid(nsim, 'sis')
...
for i in range(len(variants_plotlist)):
    ...
    sim5 = model5[variants_plotlist[i]]
    aic_min5 = np.min(sim5.aic)
    idx_min5 = np.argmin(sim5.aic)
    ...
    plt.plot(sim5.tot_avg[idx_min5], label = "SEIR",  linewidth=1.5, color = 'violet')
...
plt.tight_layout(),
my_dpi = 300
plt.savefig('aic_simulation_all.png', dpi = my_dpi)
